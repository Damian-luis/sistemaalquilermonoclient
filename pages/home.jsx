...
import { Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Chip, Select, MenuItem } from '@mui/material';
import BalanceInfo from '../componentes/BalanceInfo';
import AccountBalanceIcon from '@mui/icons-material/AccountBalance';
import { Toaster } from 'react-hot-toast';
import {formatDate} from "../utils/time";
...
import { useDevolution } from '../hooks/useDevolution';
import { useLoading } from '../hooks/useLoading';

function Home() {
  const { user } = useAuth();
  const { historical, stations, userData, fetchHistorical, activateBonus } = useHistoricalData(user);
  ...
  } = useReservation(fetchHistorical, user, userData, setLoading);
  ...
  const {
    ...
    setStationId,
  } = useDevolution(historical, fetchHistorical, setLoading);
  ...
  return (
    <div>
      <ProtectedRoute>
      <Button variant="contained" onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: '8px',backgroundColor:"#2e7d32",borderRadius:"10px",margin:"5px" }}>
      <Typography style={{fontSize:"12px"}}>Salir</Typography>
      <LogoutIcon />
    </Button>
<div
